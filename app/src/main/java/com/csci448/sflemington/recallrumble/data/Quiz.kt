...
class Quiz(val title: String = "", val questionList: List<Question> = listOf(), val category: Category = Category(0, 0, listOf()),
    ...
    fun toMutableQuiz(): MutableQuiz{
        ...
        return MutableQuiz(title = mutableStateOf(title), questionList = mutableQuestionList.toList(), category = mutableStateOf(category), creatorID = mutableStateOf(creatorID))
    }
}