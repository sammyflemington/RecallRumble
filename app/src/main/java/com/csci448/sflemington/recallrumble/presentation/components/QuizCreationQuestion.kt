...
@Composable
fun QuizCreationQuestion(question: MutableQuestion, myNumber : Int){
    val prompt = question.mPrompt
    val answers = question.mAnswerChoices
    val correctAnswerIndex = question.mCorrectAnswerIndex
...
//    }
    ...
    Row(){
        ...
        Column (Modifier.padding(10.dp)){
            QuizCreationPrompt(question, myNumber, onEditButtonClicked = {})
            for (i in 0..3){
                Row(Modifier.padding(10.dp)) {
                    QuizCreationAnswerSlot(answers[i], i, correctAnswerIndex.value, onCorrectAnswerClicked = {correctAnswerIndex.value = i})
                }
            }
        }
    }
